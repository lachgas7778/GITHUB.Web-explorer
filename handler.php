<?php
if(isset($_GET["uploadFolder"])){
    $folder = $_GET["uploadFolder"];
}else{
    $folder = "uploads";
}
if(isset($_GET["utility"])){
    $m = $_GET["utility"];
}else if(isset($_POST["utility"])){
    $m = $_POST["utility"];
}else{
    $m = "upload";
}
    ...
    foreach ($dir as $fileinfo) {
        if($fileinfo->getFilename() === $_GET["id"]){
            if(strtolower($fileinfo->getExtension()) == "png" || strtolower($fileinfo->getExtension()) == "jpg" || strtolower($fileinfo->getExtension()) == "jpeg" || strtolower($fileinfo->getExtension()) == "bmp" || strtolower($fileinfo->getExtension()) == "gif"){
                ...
                $ex = explode("\n", file_get_contents($folder."/".$fileinfo->getFilename()));
...
<?php
    ...
    if(isset($_GET["file"]) && $_GET["data"]){
        $file = $_GET["file"];
        file_put_contents($folder."/".$file, $_GET["data"]); 
    }else{
        echo false;
    }
}else if($m == "delete"){
    if(isset($_GET["filename"])){
        ...
        echo json_encode(unlink($folder."/".$filename));
    }else{
    ...
    if(isset($_GET["mode"]) && isset($_GET["name"])){
        if($_GET["mode"] == "file"){
            $handle = fopen($folder."/".$_GET["name"], "w");
            echo fclose($handle);
        }else{
            echo mkdir($folder."/".$_GET["name"]);
        }
    }else{
        echo false;
    }
